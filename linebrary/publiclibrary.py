...
def getPublicbook(Category, County,book_name):
    books = []
    if Category == 1:
        page = "1"
        sourceCode = getResponse(
            getCountyurl(County)
            + "booksearch.do?searchtype=simplesearch&search_field=TI&search_input="
            + book_name
            + "&searchsymbol=hyLibCore.webpac.search.common_symbol&execodehidden=true&showtuple=10&execode=&nowpage="
            + page
            + "#searchtype=simplesearch&search_field=TI&search_input="
            + book_name
            + "&searchsymbol=hyLibCore.webpac.search.common_symbol&execodehidden=true&nowpage="
            + page
        )
        html = bs.BeautifulSoup(sourceCode, "html.parser")
        ...
        for book, book_name, book_img, book_url in zip(
            book_detail_list, book_name_list, book_img_list, book_url_list
        ):
            ...
            bookurl = dict(
                [("網址", getCountyurl(County) + book_url.get("href").replace("\n", ""))]
            )
            book_dict = dict(
                [
                    (
                        info.text.split("：")[0],
                        (info.text.split("：")[1].replace(":", "")),
                    )
                    for info in book
                ]
            )
